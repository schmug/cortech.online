...
| `OWNER_EMAIL`         | `clodcast@cortech.online`                                            | Distinct from Cortech Daily's `cory@cortech.online` so show mail is separable. Spotify mails the submission verification code here, and Cory confirmed on 2026-08-24 that the address is live in Cloudflare Email Routing — the #193 prerequisite is met. Note the spelling: `clodcast`, matching the `schmug/clodcast` pipeline and the `clodcast.cortech.online` bucket host. It **must stay deliverable**, because Spotify re-verifies ownership through it. |
